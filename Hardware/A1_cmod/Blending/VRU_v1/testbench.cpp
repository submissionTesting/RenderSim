#define NVHLS_VERIFY_BLOCKS (VRU_v1)
#include "VRU.h"
...
public:
    ...
    void createGaussian(VRU_IN_TYPE &gaussian, 
        ...
        // Set pixel position
        gaussian.pixel_pos_x = FP32_TYPE(x_val);
        gaussian.pixel_pos_y = FP32_TYPE(y_val);
        
        // Set mean
        gaussian.mean_x = FP32_TYPE(mean_x_val);
        gaussian.mean_y = FP32_TYPE(mean_y_val);
        
        // Set inverse covariance components
        gaussian.conx = FP32_TYPE(conx_val);
        gaussian.cony = FP32_TYPE(cony_val);
        gaussian.conz = FP32_TYPE(conz_val);
        
        // Set color and opacity
        gaussian.color.r = FP32_TYPE(color_r);
        gaussian.color.g = FP32_TYPE(color_g);
        gaussian.color.b = FP32_TYPE(color_b);
        gaussian.opacity = FP32_TYPE(opacity_val);
    ...
    void run() {
        ...
        VRUInput.PushNB(gaussian2_1);
        wait(1);
        VRUInput.PushNB(gaussian2_2);
        wait(1);
        VRUInput.PushNB(gaussian2_3);
        wait(5);
        ...
        VRUInput.PushNB(gaussian3_1);
        wait(1);
        VRUInput.PushNB(gaussian3_2);
        wait(5);
    }

    void collect() {
        VRUOutput.ResetRead();
        wait(1);  // Wait for reset and initialization
