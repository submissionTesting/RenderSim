...
#include "../NPU_PE_v1/NPU_PE_v1.h"

class NPU_v1 : public match::Module {
    SC_HAS_PROCESS(NPU_v1);
public:
    ...
    NPU_v1(sc_module_name name) : match::Module(name),
                               w_in("w_in"),
                               act_in("act_in"),
                               psum_out("psum_out") {
        for (int i = 0; i < N; i++) {      // rows
            for (int j = 0; j < N; j++) {  // cols
                array[i][j] = new NPU_PE_v1(sc_gen_unique_name("npu_pe")); // Pass row and column index to PE
                array[i][j]->clk(clk);
                array[i][j]->rst(rst);
